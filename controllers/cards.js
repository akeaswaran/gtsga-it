...
exports.update = function(req, res) {
  ...
  Card.update({'_id' : id}, updates,
    function (err, numberAffected) {
      if (err) return console.log(err);
      console.log('Updated ' + numberAffected.nModified + ' cards');
      var card = req.body;
      if (card.notificationsEnabled) {
        var emailAddresses = card.assigneeEmails;
        console.log('CARD UPDATED: ' + JSON.stringify(card) + ' SENDING EMAILS TO: ' + emailAddresses);
        var statusString = '';
        ...
        console.log('Notifications not enabled for card ' + card._id);
      }
      return res.send(202);
  });
}
